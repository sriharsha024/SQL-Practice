...
-- Rename the 2024 and 2025 Orders tables
ALTER TABLE orders2024
RENAME TO ordersArchive;
ALTER TABLE orders2025
RENAME TO orders;
...
-- Create Employee table (self-referencing ManagerID foreign key)
...
-- Create Orders2024 table (set as archive later)
CREATE TABLE Orders2024 (
    ...
    Sales DECIMAL(10 , 2 ),
    CreationTime DATETIME,
    PRIMARY KEY (OrderID , ProductID , CustomerID , SalesPersonID , CreationTime),
    FOREIGN KEY (ProductID)
        REFERENCES Product (ProductID),
    FOREIGN KEY (CustomerID)
        REFERENCES Customer (CustomerID),
...
-- Insert Orders for 2024 (multiple versions of same order to track status changes)
INSERT INTO Orders2024 VALUES
...
-- Join query: orders with customer, product, and salesperson info
SELECT 
    o.orderID,
    c.firstName,
    c.lastName,
...
FROM
    ...
    product AS p ON o.productID = p.productID
        LEFT JOIN
    employee AS e ON o.salesPersonID = e.employeeID;
